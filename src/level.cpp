...
void Level::handleDifficulty() {
    if(timeElapsed > 1) { 
        ...
        waveSpeed = clamp(waveSpeedInit + (time * 4.0f), 0.0f, 600.0f);
        spawnIntervalMin = clamp(spawnIntervalMinInit - (time / 60), 0.3f, FLT_MAX);
        spawnIntervalMax = clamp(spawnIntervalMaxInit - (time / 50), 0.6f, FLT_MAX);
        waveAmp = clamp(waveAmpInit + (time / 4.0f), 0.0f, 80.0f);
...
void Level::spawnEntity(Entity* entity) {
    ...
    entity->y = getWavePointY(entity->x) + (waveHeight / 2 - 25.0f) * (entity->up ? -1 : 1);
